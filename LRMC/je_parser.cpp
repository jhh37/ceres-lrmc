...
void parseOptionsFromArgs(int argc, char* argv[], OPTIONS &options) {
  ...
  // Otherwise, parse the input arguments.
    ...
    if (options.DEBUG) {
      ...
      std::cout << "2. Size: " << options.m << " x " << options.n << std::endl;
      std::cout << "3. Rank: " << options.r << std::endl;
      std::cout << "4. Function tolerance: " << options.tol << std::endl;
      std::cout << "5. Max. no. of iterations: " << options.max_iter << std::endl;
      std::cout << "6. Number of processors: " << options.nproc << std::endl << std::endl;
      std::cout << "7. Sqrt(nu) = " << options.sqrt_nu << std::endl << std::endl;
      std::cout << "8. USE_INNER_ITERS: " << options.USE_INNER_ITERS << std::endl;
      std::cout << "9. USE_AUTO_DIFF: " << options.USE_AUTO_DIFF << std::endl;
      std::cout << "10. USE_PCA: " << options.USE_PCA << std::endl;
      std::cout << "11. DISPLAY: " << options.DISPLAY << std::endl;
